...
- **Renamed to Claude Plugboard.** Six other repositories carried the old name, one of
  them older and more visible, so anybody searching for this one found them instead. A
  plugboard is what this is: sockets you plug an account into, with meters beside them.
  The old repository URL redirects, and profiles are untouched — they live in
  `~/.claude-profiles` and were never tied to the application name. Because the
  application name is part of the install identity, the new installer sets up a separate
  app rather than upgrading the old one: uninstall "Claude Profile Manager" by hand, and
  expect app settings to start from defaults.
- The status line no longer prints a branch symbol before the branch name. The obvious
  candidates are missing from the fonts terminals actually ship with, and a replacement
  box reads as a bug.
- A social preview card, generated by `tools/make-social.js` from an HTML file — the same
  approach as the icons, no image toolchain.
